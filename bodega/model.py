...
class Robot(Agent):
    ...
    def step(self):
        ...
        if self.sig_pos == self.pos and self.action != "cargar" and not self.wait: 
...
class Bodega(Model):
    def __init__(self, M: int = 50, N: int = 25,
        ...
        if (self.num_robots <= 6):
            ...
            self.positions_chargers = [(int(M / 2) + i, j) for i in range(-3, 5, 2) for j in [0, N - 1]]
